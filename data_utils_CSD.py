...
def genSpoof_list( dir_meta,is_train=False,is_eval=False):
    ...
    if (is_train):
        for line in l_meta:
             _,key,_,attack,label = line.strip().split()
             attack_mapping = {'-': 0, 'A01': 1, 'A02': 2, 'A03': 3,'A04': 4,'A05': 5,'A06': 6}  # Add mappings as needed
             attack_list[key] = attack_mapping.get(attack, 0)  # Default to 0 if `attack` isn't found
             file_list.append(key)
             d_meta[key] = 1 if label == 'bonafide' else 0
        return d_meta,file_list,attack_list
...
class Dataset_ASVspoof2021_eval(Dataset):
	...
	def __getitem__(self, index):
            ...
            x_inp = Tensor(X_pad)
            return x_inp,utt_id,0  


class Dataset_ASVspoof2019_train_DA(Dataset):
    def __init__(self, args, list_IDs, labels, domains,is_set, algo):
        '''self.list_IDs  : list of strings (each string: utt key),
    ...
    def __getitem__(self, index):
        ...
        domain = self.domains[key1]
        # Load the audio files
        X1, fs = librosa.load(f'./datasets/LA/ASVspoof2019_LA_{self.is_set}/flac/{key1}.flac',sr=16000)
